...
impl RegistrationResponse {
    pub fn verify(
        ...
        let client_data_json =
            serde_json::from_slice::<ClientData>(client_data).map_err(|_| Error::ParseError)?;
        ...
        let (parsed, _) =
            x509::parse_der(&self.attestation_certificate).map_err(|_| Error::ParseError)?;
        let cert = x509::Certificate::from_value(parsed);

        let msg_len = 1 + 32 + 32 + self.key_handle.len() + 65;
        let appid_hash =
            ring::digest::digest(&ring::digest::SHA256, client_data_json.origin.as_bytes());
...
fn test_reg_resp_verify() {
    ...
    let res = reg_resp.verify(
        vec!["https://localhost:2020".to_string()],
        "vfegbegi",
        &client_data,
    );
    assert!(res.is_err());
    ...
    let res = reg_resp.verify(
        vec!["https://localghost:2020".to_string()],
        "dfgbhnjklm,fcghjnm",
        &client_data,
    );
    assert!(res.is_err());
...
fn test_parse_registration_response() {
    ...
    assert_eq!(
        res.user_public_key,
        vec![
            0x04, 0x74, 0x0f, 0x1d, 0xd9, 0xcb, 0x87, 0xd8, 0xef, 0x8c, 0x5f, 0x4b, 0x4c, 0x19,
            0x23, 0xd1, 0x64, 0xcc, 0x5b, 0x6d, 0x88, 0xd0, 0x3f, 0x14, 0xe3, 0x83, 0x31, 0xf6,
            0x02, 0x0a, 0x1d, 0x97, 0xa7, 0x1d, 0xf5, 0xdb, 0xa7, 0xe4, 0xa3, 0x7c, 0xc1, 0x1b,
            0x0c, 0xad, 0x1b, 0x3a, 0xc3, 0x99, 0x09, 0xf7, 0x57, 0x38, 0x04, 0x1b, 0xe8, 0xb3,
            0x3b, 0x49, 0x26, 0x77, 0xc9, 0x0d, 0xa6, 0xcd, 0x6a
        ]
    );
    assert_eq!(
        res.key_handle,
        vec![
            0x82, 0x1a, 0x01, 0xaa, 0xea, 0x50, 0x5e, 0x02, 0x01, 0x14, 0x18, 0xa9, 0xa4, 0xc5,
            0x04, 0x82, 0x36, 0x7a, 0xa7, 0x22, 0x26, 0x94, 0x2f, 0xaa, 0xcd, 0xa1, 0xae, 0x91,
            0x27, 0xb6, 0x11, 0xc8, 0x4b, 0xf7, 0xb0, 0xfa, 0xe7, 0xf2, 0xdb, 0x95, 0xf8, 0x49,
            0x44, 0x1a, 0x3d, 0x13, 0xeb, 0x81, 0x10, 0xe6, 0x1a, 0x84, 0x98, 0x28, 0x72, 0x02,
            0x81, 0x52, 0x62, 0x3a, 0xaa, 0x75, 0xce, 0xd3
        ]
    );
    assert_eq!(
        res.attestation_certificate,
        vec![
            
        ]
    );
    assert_eq!(
        res.signature,
        vec![
            0x30, 0x46, 0x02, 0x21, 0x00, 0xb6, 0x58, 0xa2, 0x3b, 0xcb, 0xde, 0x95, 0x07, 0xb8,
            0x2b, 0x0f, 0x82, 0x0b, 0x47, 0x53, 0xba, 0x97, 0xb5, 0x53, 0xd8, 0xa3, 0x44, 0x9a,
            0x13, 0x89, 0x52, 0xb0, 0x9f, 0x99, 0x8b, 0x75, 0x4e, 0x02, 0x21, 0x00, 0xbf, 0x07,
            0x71, 0xfb, 0x08, 0xdf, 0x8a, 0xa2, 0xfe, 0xba, 0x09, 0x79, 0x37, 0x92, 0xde, 0x0b,
            0x1c, 0x06, 0xb6, 0xfa, 0xab, 0x4a, 0x39, 0x91, 0x0f, 0x56, 0x03, 0xb7, 0xd7, 0xde,
            0xe4, 0x6c
        ]
    );
}
...
fn test_signature_data_verify() {
    let pub_key = base64::decode_config(
        "BHQPHdnLh9jvjF9LTBkj0WTMW22I0D8U44Mx9gIKHZenHfXbp-SjfMEbDK0bOsOZCfdXOAQb6LM7SSZ3yQ2mzWo",
...
fn test_parse_signature_data() {
    ...
    assert_eq!(
        vec![
            0x30, 0x46, 0x02, 0x21, 0x00, 0xc2, 0x19, 0x91, 0xf4, 0xf7, 0x57, 0xce, 0x37, 0x4e,
            0x30, 0x7f, 0xd0, 0xba, 0x5e, 0xeb, 0xda, 0x92, 0x0f, 0xef, 0xa6, 0x75, 0x15, 0x45,
            0x65, 0x35, 0x4d, 0x6b, 0x4d, 0x6e, 0xd3, 0x54, 0x01, 0x02, 0x21, 0x00, 0xc3, 0xdb,
            0x0a, 0xc0, 0x77, 0xbb, 0x95, 0xff, 0xe1, 0x94, 0xde, 0xb7, 0x50, 0x7e, 0xe0, 0xf2,
            0x4d, 0x01, 0x52, 0x0c, 0xec, 0x96, 0x84, 0xa4, 0x14, 0xf3, 0xa4, 0xff, 0x7e, 0x2e,
            0x8c, 0x49
        ],
        sig.signature
    );
